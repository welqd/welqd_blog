...
Также можно автоматически отправлять терминал на нужный рабочий стол (к примеру, на второй монитор) и открывать его в tabbed layout.  

Разберем, как этого достичь.  

При создании нового терминала Allacritty значение в xprop **WM_NAME(STRING) = "Alacritty"**, а при открытии устройства из EVE-NG **WM_NAME(STRING)** будет такое же, как и название устройства.  
Можно этим воспользоваться и добавить в конфигурационный файл i3 следующую строку:  
```
for_window [class="Alacritty" title="^(?!Alacritty).*"] move to workspace $ws7 layout tabbed
```
Что, собственно, эта строка делает:  
Для любого терминала Alacritty, название которого не равно **Alacritty** (это условие будет выполняться при открытии устройств в EVE-NG), перемещать его на workspace 7 и открывать в tabbed layout.  

Соотвественно мы получаем "аналог" терминала со вкладками на отдельном рабочем столе.  
