...
В [прерыдущей]({{< ref "/post/eve-ng-arch-i3.md" >}}) публикации был установлен образ EVE-NG и настроена базовая конфигурация.Теперь создадим образ Linux-роутера на основе Debian 11 для экспериментов и добавим его в EVE-NG.   

## Создание вируальной машины

[Скачиваем](https://cdimage.debian.org/debian-cd/current/amd64/bt-cd/) образ с официального сайта.  
Необходимо создать виртуальную машину, для этих целей будет использоваться VMware Workstation. 
В процессе настройки ВМ на этапе указания объема диска **обязательно** выбрать **Store virtual disk as a single file**.  
Т.к. необходим наиболее минимальный образ, то объем диска указываем 4 Гб, этого достаточно.  
...
[^1]: В качестве основы для данной публикации использовалась [данная](https://www.brianlinkletter.com/2017/03/build-custom-linux-router-image-unetlab-eve-ng-network-emulators/) статья, с необходимыми изменениями в программах образа (к примеру, использование frr) и исправлениями в шаблонах (ранее EVE-NG использовали php, в момент написания статьи - yaml)
